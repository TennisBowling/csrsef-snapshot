...
from scheduler import AsyncHyperBandScheduler, run_trial
...
def train(trial_id, train_dataset, test_dataset, num_node_types, config, epoch_callback, device_id):
    device = torch.device(f"cuda:{device_id}" if torch.cuda.is_available() else "cpu")

    wandb_run = wandb.init(
        project="sann_code_classifier",
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if epoch_callback is not None:
            ...
            continue_training = epoch_callback(epoch + 1, metrics)
    ...
    checkpoint = torch.load(f"/gscratch/scrubbed/enzovt/trial_{trial_id}.pt")
...
async def main():
    ...
    config_space = {
        "embedding_dim": {"values": [64, 128, 256], "default": 128},
        "num_heads": {"values": [4, 8, 16], "default": 8},
        "num_layers": {"values": [2, 4, 6], "default": 4},
        "dropout": {"min": 0.1, "max": 0.5, "type": "float", "default": 0.3},
        "batch_size": {"values": [4, 8, 16, 32], "default": 8},
        "lr": {"min": 1e-7, "max": 1e-5, "type": "log", "default": 1e-6},
        "weight_decay": {"values": [1e-4, 1e-3, 1e-2], "default": 1e-3},
        "label_smoothing": {"values": [0.0, 0.1, 0.2], "default": 0.1},
        # num_epochs will be managed by the scheduler
    }
    ...
    scheduler = AsyncHyperBandScheduler(
        ...
        num_trials=100,
        metric="accuracy",     # Look to "f1" in the future
    ...
    with cfutures.ThreadPoolExecutor() as executor:
        for i in range(9):
            print("starting", i)
            trial_id, config = scheduler.get_next_trial()
            running_trials += 1
            logger.info(f"Starting trial {trial_id} with config: {config}")
            x = functools.partial(run_trial, scheduler, train, train_dataset, test_dataset, len(extractor.node_types), running_trials - 1, trial_id, config)
            futures.append(asyncio.get_running_loop().run_in_executor(executor, x))
            
    await asyncio.gather(*futures)
    
    # Log final results
    best_result = scheduler.get_best_result()
    logger.info("Hyperparameter optimization completed:")
    logger.info(f"Best configuration (Trial {best_result['trial_id']}):")
