...
def run_epoch(model, dataloader, optimizer, criterion, device, is_training=True):
        ...
        for subtrees, labels in tqdm(dataloader):
            try:
                ...
                if is_training:
                    ...
                    loss.backward()
                    # -----------------------------------------------------------
                    # REPLACE HARD-CODED CLIP WITH THE ONE FROM CONFIG["grad_clip"]
                    # -> We will inject the actual value in the 'train' function.
                    # -----------------------------------------------------------
                    torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)  
                    # (Will be overridden in the 'train' function)
                    optimizer.step()
    ...
    if len(predictions) > 0:
        ...
        # Approximation for average batch loss:
...
def train(train_dataset, test_dataset, num_node_types, config):
    ...
    save_dir = os.path.join("/gscratch/scrubbed/enzovt/results", trial_id)
    ...
    # We will capture the final epoch's validation metrics.
    ...
    for epoch in range(config["num_epochs"]):
        model.train()
        # -------------------------------
        # Here we inject our grad_clip:
        # -------------------------------
        for param_group in optimizer.param_groups:
            pass  # Just ensures optimizer is built before we patch run_epoch

        old_run_epoch = run_epoch
        def patched_run_epoch(*args, **kwargs):
            with torch.set_grad_enabled(kwargs.get("is_training", True)):
                # We'll do a local override of run_epoch logic:
                total_loss = 0
                predictions = []
                labels_list = []
                ...
                data_loader = args[1]
                local_model = args[0]
                local_optimizer = args[2]
                local_criterion = args[3]
                local_device = args[4]
                local_is_training = kwargs.get("is_training", True)

                print("patched run device:", local_device)
                print("cuda visible devices", os.environ.get("CUDA_VISIBLE_DEVICES"))
                
                for subtrees, labels in tqdm(data_loader):
                    try:
                        subtrees = subtrees.to(local_device)
                        labels = labels.to(local_device)
                        
                        outputs, _ = local_model(subtrees)
                        loss = local_criterion(outputs, labels)
                        
                        if local_is_training:
                        ...
                        logger.error(f"Error processing batch (patched): {str(e)}")
                        if "CUDA error" in str(e) or "device-side assert" in str(e):
                            logger.warning("CUDA error detected, skipping batch")
                            if local_is_training:
                                local_optimizer.zero_grad()
                ...
                if len(predictions) > 0:
                    accuracy = np.mean(np.array(predictions) == np.array(labels_list))
                    f1 = calculate_macro_f1(predictions, labels_list)
                    # Approx. for average batch loss:
                    avg_loss = total_loss / max(1, len(predictions) // labels.size(0))
                else:
                    logger.warning("No batches were successfully processed in this epoch (patched).")
                    accuracy = 0.0
                    f1 = 0.0
                    avg_loss = float('inf')
            ...
            return old_run_epoch(*args, **kwargs)
        
        # Use the patched function for training
        train_loss, train_acc, train_f1 = patched_run_epoch(
            model, train_dataloader, optimizer, criterion, device, is_training=True
        )
        
        # Validation (no grad clip needed)
        model.eval()
        val_loss, val_acc, val_f1 = old_run_epoch(
            model, val_dataloader, None, criterion, device, is_training=False
        )
        
        # Test
        test_loss, test_acc, test_f1 = old_run_epoch(
        ...
        # Track final epoch's val metrics
        final_val_loss = val_loss
        final_val_acc = val_acc
        final_val_f1 = val_f1

        # Early stopping - save best model for this trial
        if val_f1 > best_val_f1:
            ...
            logger.info(f"Saved new best model with validation F1: {val_f1:.4f} to {best_checkpoint_path}")
        else:
            patience_counter += 1
        ...
        epoch_checkpoint_path = os.path.join(save_dir, f"checkpoint_epoch_{epoch}.pt")
    ...
    final_checkpoint = {
        ...
        "node_types": train_dataset.extractor.node_types
    }
    ...
    return {"model": model, "test_acc": test_acc, "test_f1": test_f1, "final_val_loss": final_val_loss, 
            "final_val_acc": final_val_acc, "final_val_f1": final_val_f1, "test_loss": test_loss}


def save_confusion_matrix(y_true, y_pred, save_dir, name="confusion_matrix"):
    ...
    confusion_matrix_path = os.path.join(save_dir, f'{name}.png')
    ...
    np.save(os.path.join(save_dir, f'{name}.npy'), cm)
...
def train_sann(config, train_dataset, test_dataset, num_node_types):
    model, final_test_acc, final_test_f1, final_val_loss, final_val_acc, final_val_f1, final_test_loss = \
        train(train_dataset, test_dataset, num_node_types, config)
    # Report to Ray Tune
    tune.report(
        val_loss=final_val_loss,
        val_acc=final_val_acc,
...
def main():
    ...
    tuner = tune.run(
        ...
        callbacks=[
            WandbLoggerCallback(
                project="SANN_RayTune2",
                log_config=True
            )
        ],
        storage_path="/gscratch/scrubbed/enzovt/raytune2/",
        name="SANN_RayTune2",
        resume="AUTO",
        max_failures=1,
    )

    # If desired, print out the best result
    best_result = tuner.get_best_trial("val_f1", "max", "last")
    logger.info(f"Best trial config: {best_result.config}")
    logger.info(f"Best trial final validation F1: {best_result.last_result['val_f1']:.4f}")
    logger.info(f"Best trial final test F1: {best_result.last_result['test_f1']:.4f}")
