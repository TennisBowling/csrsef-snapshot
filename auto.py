...
def create_confusion_matrix_plot(y_true, y_pred, phase="train"):
    ...
    ax.set(xticks=np.arange(cm.shape[1]),
           ...
           title=f'Confusion Matrix - {phase.capitalize()} Phase',
           ylabel='True label',
           xlabel='Predicted label')
    
    # Rotate xtick labels and align
...
def run_epoch(model, dataloader, optimizer, criterion, device, is_training=True):
    ...
    cm, cm_plot = create_confusion_matrix_plot(
        np.array(labels_list), 
        np.array(predictions),
        "train" if is_training else "validation"
    )
    
    return total_loss / len(dataloader), accuracy, f1, cm, cm_plot
...
def train_tune(config, checkpoint_dir=None, data_dict=None):
    ...
    for epoch in range(config["num_epochs"]):
        # Training phase
        model.train()
        train_loss, train_acc, train_f1, train_cm, train_cm_plot = run_epoch(
            model, train_dataloader, optimizer, criterion, device, is_training=True
        )
        
        # Validation phase
        val_loss, val_acc, val_f1, val_cm, val_cm_plot = run_epoch(
            model, val_dataloader, None, criterion, device, is_training=False
        )
        
        # Test phase
        test_loss, test_acc, test_f1, test_cm, test_cm_plot = run_epoch(
        ...
        train.report({
            ...
            "test_confusion_matrix": test_cm_plot
        })
...
def evaluate_tune(model, dataloader, criterion, device):
    ...
    cm, cm_plot = create_confusion_matrix_plot(all_labels, all_preds, "test")

    return total_loss / len(dataloader), accuracy, macro_f1, cm, cm_plot
...
def main():
    ray.init(
        num_gpus=4,
        _memory=int(300e9),  # Reserve 200GB RAM for Ray
        _redis_max_memory=int(20e9),  # 10GB for Redis
        #_temp_dir="/gscratch/scrubbed/enzovt/raytemp/"
    )
    ...
    tuner = tune.Tuner(
        ...
        run_config=tune.RunConfig(
            name="sann_tuning",
            storage_path="file:///gscratch/scrubbed/enzovt/ray_results",
