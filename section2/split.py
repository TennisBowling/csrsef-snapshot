...
class CodeSplitter:
    ...
    def extract_code_blocks(self, code: str, full_path: str) -> List[tuple]:
        try:
            tree = ast.parse(code)
        except:
            return []
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                if current_lines:
                    ...
                    try:
                        ...
                        if (self.is_valid_block(parsed_block) and 
                            ...
                            blocks.append((standalone_block, proc_code, full_path))
                    except:
                        pass
                    current_lines = []
                ...
                try:
                    ...
                    if node_code and self.is_valid_block(node):
                        ...
                        blocks.append((node_code, proc_code, full_path))
                except:
                    continue
            else:
                try:
...
def main():
    ...
    for orig_code, proc_code, full_path in all_blocks:
        duplicates[proc_code].append((orig_code, full_path))
    ...
    for blocks in duplicates.values():
        final_blocks.append({'code': blocks[0][0], 'full_path': blocks[0][1]})
    ...
    result_df.to_parquet('github_split.parquet', index=False)
