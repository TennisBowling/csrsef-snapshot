...
class LLMClient:
    def __init__(self):
        self.api_key = "sk-or-"
        self.client = AsyncOpenAI(api_key=self.api_key, timeout=300, base_url="https://openrouter.ai/api/v1")
    ...
    async def send_req(self, code_content: str) -> str | None:
        ...
        try:
            response = await self.client.chat.completions.create(
                ...
                temperature=0.7,
                extra_headers={
                    "HTTP-Referer": "https://tennisbowling.com",
                    "X-Title": "TennisBowling Research Project",
                },
                extra_body={}
            )
    ...
    async def do_work(self):
        ...
        if job["command"] == "process_batch":
            ...
            code_content = job["data"][0]["code"]
            full_path = job["data"][0]["full_path"]

            response = await self.send_req(code_content)
            if not response:
                async with self.lock:
